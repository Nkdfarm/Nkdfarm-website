...
const FarmSimTest = () => {
  ...
  useEffect(() => {
    ...
    document.querySelectorAll(".fst-fade").forEach((el) => observerRef.current?.observe(el));
  ...
  return (
    <div className="min-h-screen bg-background animate-fade-in">
      ...
      {/* ── FOOTER BAR ─────────────────────────────── */}
      <footer className="bg-foreground text-primary-foreground/50 px-6 py-8 text-xs flex flex-wrap justify-between gap-4">
        <span><strong className="text-primary-foreground">FarmBox Cape Town</strong> — Hydroponic Yield & Demand Model V5 · Draft · March 2026</span>
        <span>Generated from live model data · All figures indicative and subject to operational assumptions</span>
      </footer>
    </div>
  );
};

export default FarmSimTest;
